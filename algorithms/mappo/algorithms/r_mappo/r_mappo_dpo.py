...
class R_MAPPO():
    ...
    def __init__(self,
        ...
        if self.args.dynamic_clip_tag:
            self.clip_param = np.ones(self.args.num_agents)*args.clip_param
            ...
            self.min_clip_constant = np.log(self.args.min_clip_params + 1) * self.args.num_agents
        ...
        if self.args.penalty_method:
            ...
            if self.args.env_name == 'mujoco':
                ...
                if self.args.env_name == 'StarCraft2':
                    ...
                    print("env name",self.args.env_name)
            ...
            # kl and obs_batch is with shape (episode_length * num_agents, n_actions)
            ...
            policy_active_masks_batch = active_masks_batch.reshape([-1,self.args.num_agents])
            ...
            if self.args.dynamic_clip_tag:
                if self.args.idv_para and aga_update_tag and self.args.aga_tag:
                    ...
                    surr1 = []
                    surr2 = []
                    agent_adv_targ = adv_targ.reshape([-1, self.args.num_agents, 1])
                    if self.args.env_name == 'mujoco':
                        old_action_log_probs_batch = old_action_log_probs_batch.mean(axis = -1)
                    agent_old_action_log_probs = old_action_log_probs_batch.reshape([-1, self.args.num_agents, 1])
        ...
        if self.args.dynamic_clip_tag:
            ...
            if self.args.dcmode == 1:
                ...
                if self.args.delta_reset:
                    idv_delta = self.curr_constant / self.args.num_agents
                    idv_eps = np.exp(idv_delta) - 1
                    self.clip_param = np.ones(self.args.num_agents) * idv_eps
