...
class Scenario(BaseScenario):
    def action_callback(self, agent, _): 
      ...
      return agent.action
    def make_world(self, N=2,penalty_ratio=0.5, full_comm=False):
        world = World()
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            ...
            comm.append(other.state.p_pos - message[:-1])
            other_pos.append(other.state.p_pos - agent.state.p_pos)
        
        if not agent.adversary:
            
            # only communication in good agents
            return np.concatenate(
                [agent.goal_a.state.p_pos - agent.state.p_pos] + entity_pos + comm
            )
        else:
